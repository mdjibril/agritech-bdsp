...
# Phase 1 & 2: Database, API & WhatsApp
...
#### 6c. Login API works through the proxy
...
#### 6d. BDSP network endpoint works through the proxy
...
#### 6e. Verify 70/30 commission split through the proxy
...
#### 6f. Confirm non-BDSP users don't see the BDSP navigation
...
## 7. Phase 6: Deploy to Render

### 7a. Production config hardening
...
### 7b. Required environment variables
...
### 7f. Post-deployment verification checklist

- [ ] `curl https://agritech-bdsp-back.onrender.com/health` returns `{"status":"ok","database_time":"..."}`
- [ ] Login works: `curl -X POST ... /auth/login` returns a user + token
- [ ] Marketplace returns posts: `curl ... /posts?status=Active`
- [ ] Frontend loads at `https://agritech-bdsp-frontend.onrender.com` with no console errors
...
### 7g. CI/CD — Automated deploy with GitHub Actions
