...
export default function ReportsView({ user }) {
  ...
  const [bdsps, setBdsps] = useState([]);

  useEffect(() => {
    apiV1('/actors').then((r) => {
      setBdsps((r.actors || []).filter((a) => a.actor_type === 'BDSP'));
    }).catch(() => {});
  }, []);
    ...
    <Page title="Reports" subtitle="Performance reports and data exports">
      <div className="panel" style={{ marginBottom: 20 }}>
        ...
        {(report === 'completed' || report === 'financial') && (
          <div className="filter-group" style={{ margin: '12px 0', display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {report === 'completed' && (
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                ...
                <select value={genderFilter} onChange={(e) => setGenderFilter(e.target.value)}
                  style={{ padding: '6px 10px', borderRadius: 4, border: '1px solid var(--line)', fontSize: 12 }}>
      ...
      {data && report === 'completed' && (() => {
        ...
        const filteredV4V = Number(filteredTxs.reduce((s, r) => s + Number(r.commission_v4v || 0), 0).toFixed(2));
        const filteredBdsp = Number(filteredTxs.reduce((s, r) => s + Number(r.commission_bdsp || 0), 0).toFixed(2));
        return (
        <div className="panel" style={{ marginBottom: 20 }}>
          <div className="panel-head">
            <div><h2>Completed Transactions</h2><p>{data.summary.total_count} deals completed — showing {filteredTxs.length}</p></div>
            <button className="secondary-button sm" onClick={() => downloadCsv('completed-transactions.csv', csv(filteredTxs, [
              ...
              { label: 'Seller Gender', get: (r) => r.seller_gender || '' },
              { label: 'Seller BDSP', get: (r) => r.seller_bdsp_id || '' },
          ...
          <div className="metrics-grid" style={{ marginBottom: 16 }}>
            <div className="metric"><div className="metric-icon"><PackageOpen size={20} /></div><div><span>Total volume</span><strong>{filteredVolume.toLocaleString()} kg</strong></div></div>
            <div className="metric"><div className="metric-icon"><TrendingUp size={20} /></div><div><span>Total value</span><strong>{money(filteredValue)}</strong></div></div>
            <div className="metric"><div className="metric-icon"><Shield size={20} /></div><div><span>V4V revenue</span><strong>{money(filteredV4V)}</strong></div></div>
            <div className="metric"><div className="metric-icon"><Users size={20} /></div><div><span>BDSP commissions</span><strong>{money(filteredBdsp)}</strong></div></div>
          </div>
          {filteredTxs.length === 0 ? <Empty /> : (
            <div className="table-wrap">
              <table>
                <thead><tr><th>ID</th><th>Commodity</th><th>Qty</th><th>Amount</th><th>Buyer</th><th>Seller</th><th>Gender</th><th>BDSP</th><th>Logistics</th><th>V4V</th><th>BDSP Fee</th><th>Date</th></tr></thead>
                <tbody>
                  {filteredTxs.map((t) => (
                    <tr key={t.tx_id}>
                      ...
                      <td>{t.buyer_name}</td>
                      <td>{t.seller_name}</td>
                      <td>{t.seller_gender || '—'}</td>
                      <td>{t.seller_bdsp_id || '—'}</td>
      ...
      {data && report === 'farmers' && (() => {
        ...
        <div className="panel" style={{ marginBottom: 20 }}>
          <div className="panel-head">
            <div><h2>Farmer Participation</h2><p>{data.summary.total_shfs} registered SHFs — showing {filteredFarmers.length}</p></div>
            <button className="secondary-button sm" onClick={() => downloadCsv('farmer-participation.csv', csv(filteredFarmers, [
          ...
          {filteredFarmers.length === 0 ? <Empty /> : (
            <div className="table-wrap">
              <table>
                <thead><tr><th>ID</th><th>Name</th><th>Phone</th><th>Gender</th><th>LGA</th><th>BDSP</th><th>Active</th><th>Repeat</th><th>Registered</th></tr></thead>
                <tbody>
